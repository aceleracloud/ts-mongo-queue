...
const mockNow = new Date('2023-10-05T03:14:48.786Z')
...
jest.mock('../../src/v2/helpers', () => ({
  now: jest.fn().mockReturnValue(new Date('2023-10-05T03:14:48.786Z')),
  nowPlusSecs: jest.fn().mockReturnValue(new Date('2023-10-05T03:15:48.786Z')),
...
describe('QueueV2', () => {
  ...
  it('should move message to deadQueue after exceeding maxRetries', async () => {
    ...
    const queue = new QueueV2(dbMock as any, 'test-queue', { deadQueue: deadQueueMock as any, maxRetries: 5 })
  ...
  it('should ping (update visibility of) a message', async () => {
    ...
    expect(collectionMock.findOneAndUpdate).toHaveBeenCalledWith({ ack: mockMsg.ack, deleted: null }, { $set: { visible: expect.any(Date) } }, { returnDocument: 'after' })
  })
  ...
  it('should return the size of the queue', async () => {
    ...
    expect(collectionMock.countDocuments).toHaveBeenCalledWith({ deleted: null, visible: { $lte: mockNow } })
